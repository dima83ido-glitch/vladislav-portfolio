...
export type NormalizedService = {
  id: string;
  /** Icon name (string) — resolved to a component here, never crossed the server/client boundary as a function. */
  icon: string;
  index: string;
  title: string;
  description: string;
...
export function ServicesGrid({ services }: { services: NormalizedService[] }) {
  return (
          ...
          <Reveal key={service.id} variant="up" delay={(i % 3) * 0.1}>
            ...
            >
              ...
              <div className="relative flex items-start justify-between">
                ...
                <span className="font-mono text-xs text-muted/60">{service.index}</span>
              </div>
